...
\*Error boundary help us to handle runtime errors from javascript and they work
as a try catch for components. Error boundaries will catch all errors that
ocurred in its children

\*We can have a unique error boundary that wrapps everything in our app, but
that is not flexible, so usually is better to use multiple error bounaries
wrappers well allocated in our app.

\*react-error-boundary Is a librar to handle error boundaries.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=agustin.moranr%40gmail.com).
