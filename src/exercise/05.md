...
\*When we are adding event listeners, methods, etc, to our elements, it is
important to cleanUp them. Because if the component its unmounted those
listeners will continue existing in browser memory, so they eventually become in
a memory leak.

\*useRef Hook is helpful to create a ReactElement reference, but also to create
a reference to an specific value, and the main thing is that when we update that
value we dont force a re-render of the componente. Also this value will persists
after re-renders so useRef can be very useful
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=agustin.moranr%40gmail.com).
